<nav>
    <dev id="left-menu">
        <img id="main-icon" src="public/assets/logo-full-login-page.svg" alt="ShopTherapyy icon" ></img>
        <img id="mobile-icon" src="public/assets/logo-mobile-login-page.png" alt="ShopTherapyy icon"></img>
        <dev id="menu-items">
            <a id="about-item" class="menu-item">about</a>
            <a id="blog-item" class="menu-item">blog</a>
            <a id="FAQ-item" class="menu-item">FAQ</a>
        </dev>
    </dev>

    <dev id="right-menu">
        <button id="nav-sign-up">Sign up</button>
        <button id="nav-log-in">Log in</button>
    </dev>
</nav>
